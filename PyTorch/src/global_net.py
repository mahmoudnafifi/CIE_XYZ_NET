...
class globalSubNet(nn.Module):
    def __init__(self, blockDepth=5, convDepth=64, in_img_sz=128, device='cuda'):
        ...
        for i in range(self.blockDepth):
            if i == 0:
                ...
                conv = torch.nn.Conv2d(self.convDepth, self.convDepth, 3, padding=1)
                torch.nn.init.kaiming_normal_(conv.weight)
                torch.nn.init.zeros_(conv.bias)
            self.net.add_module('conv%d' % i, conv)
            self.net.add_module('leakyRelu%d' % i, torch.nn.LeakyReLU(inplace=False))
            self.net.add_module('maxpool%d' % i, torch.nn.MaxPool2d(2, stride=2))
    ...
    def forward(self, x):
        inds_1 = torch.LongTensor(np.linspace(0, x.size(2), self.in_img_sz, endpoint=False)).to(device=self.device)
        inds_2 = torch.LongTensor(np.linspace(0, x.size(3), self.in_img_sz, endpoint=False)).to(device=self.device)
