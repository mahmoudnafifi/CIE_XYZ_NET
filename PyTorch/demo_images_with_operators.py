...
def get_args():
    ...
    parser.add_argument('--input_dir', '-i', help='Input image directory',
                        dest='input_dir', default='../images/')
    parser.add_argument('--pp_method', '-t',
                        default='none|none|denoise+deblur|none|none',
                        help="Post-processing methods; denoise, deblur,"
                             " dehaze, editdetails,"
                             " exposure-fusion, transfer-colors, chrom-adapt,"
                             " super-res. The order is: "
...
if __name__ == "__main__":
    ...
    for filename in imgfiles:
        ...
        with torch.no_grad():
            ...
            if tasks[2] != 'none':
                xyz = pp.postprocessing(xyz, tasks[2]).to(
                    device=device, dtype=torch.float32)

            srgb = ciexyzNet.forward_global(xyz, target='srgb')

            if tasks[3] != 'none':
                srgb = pp.postprocessing(srgb, tasks[3]).to(
                    device=device, dtype=torch.float32)
